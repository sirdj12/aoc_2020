...
int main() {
    ...
    for (int i = 0; ((i < number_of_lines) && flag); i++){
        for (int j = i; (j < number_of_lines) && flag; j++){
            for (int k = j; (k < number_of_lines) && flag; k++){
    ...
    cout << "The numbers are " << nums[index1] << ", " << nums[index2] << ", and " << nums[index3] << ", and their product is " << nums[index1]*nums[index2]*nums[index3] << endl;
